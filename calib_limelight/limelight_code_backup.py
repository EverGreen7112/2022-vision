...
cam_angle = 48
denoise_hub = gbv.MedianBlur(3) + gbv.Dilate(5, 2) + gbv.Erode(5, 2) + gbv.DistanceTransformThreshold(0.166)
...
cam = gbv.USBCamera(settings.CAMERA_PORT, gbv.CameraData(777.3291449774972,1.0402162342 , 0.86742863824, pitch_angle=math.radians(cam_angle), name="limelight"))
...
hub_obj = track_object(cam=cam, pid_vals=[94, 213, 216],
                        hue=[settings.HUE_KP, settings.HUE_KI, settings.HUE_KD], sat=[settings.SAT_KP, settings.SAT_KI, settings.SAT_KD], val=[settings.VAL_KP, settings.VAL_KI, settings.VAL_KD],
                        range=[10, 110, 90], target= gbv.GameObject(0.08868483523128404), denoise_pipe = denoise_hub)
...
def runPipeline(image, llrobot):
    ...
    locals[1] = locals[1] * 72 / 96
    # distance = math.sqrt(locals[1]**2+locals[2]**2)
    # locals[2] = math.cos(math.radians(cam_angle))*distance
    # locals[1] = math.sin(math.radians(cam_angle))*distance
